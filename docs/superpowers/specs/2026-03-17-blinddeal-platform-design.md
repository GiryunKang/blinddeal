...
### 비전
**BlindDeal** — 부동산과 M&A 딜을 공개/비공개로 중개하는 전문 거래 플랫폼. 개인 투자자/자산가(A)와 기업/법인(B)이 직접 만나되, 검증된 신뢰 체계와 풀 프로세스(매칭→협상→실사→계약→에스크로) 위에서 거래한다.

### 핵심 차별점
- **공개/비공개 이중 구조**: 등록자가 선택, 비공개 딜은 자격 검증된 사용자에게만 노출
- **비대칭 인증**: 공급자(매도자)는 점진적 경량 인증, 수요자(매수자)는 등급별 자격 검증
- **풀 프로세스 + 전문가 네트워크**: 매칭→협상→실사→계약→에스크로 + 법무/회계/세무 전문가 연결
- **체류 유도 콘텐츠**: 시장 인사이트 + 커뮤니티 + 맞춤 매칭/알림

### 수익 모델
1. **중개 수수료**: 딜 성사 시 거래 금액의 일정 비율
2. **멤버십**: 비공개 딜 접근, 프리미엄 리포트, 우선 매칭 등 유료 구독
