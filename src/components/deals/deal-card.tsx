...
export function DealCard({ deal, isBlind = false, index = 0 }: DealCardProps) {
  ...
  return (
    <Link href={isBlind ? "#" : `/deals/${deal.slug}`} className="group block">
      <div
        className={`deal-card relative rounded-2xl overflow-hidden bg-card border transition-all duration-500 ease-out hover:-translate-y-1.5 ${
          isBlind
            ? "watermark-confidential border-red-500/15 hover:shadow-[0_20px_60px_-15px_rgba(239,68,68,0.12)] hover:border-red-500/40"
            : "border-border/50 hover:shadow-[0_20px_60px_-15px_rgba(59,130,246,0.18)] hover:border-blue-500/30"
        }`}
      ...
      >
        ...
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          {deal.thumbnail_url ? (
            ...
            <div
              className={`w-full h-full bg-gradient-to-br from-card via-muted to-muted/80 flex items-center justify-center transition-transform duration-500 ease-out group-hover:scale-105 ${
                isBlind ? "blur-md" : ""
              }`}
            >
        ...
        {isBlind && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/50 backdrop-blur-sm">
            ...
            <div className="relative flex h-16 w-16 items-center justify-center rounded-full border border-white/10 bg-white/5 backdrop-blur-md mt-3">
